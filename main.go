...
import (
	...
	pb "github.com/dsrvlabs/vatz-plugin-eth2.0/plugin"
	"github.com/dsrvlabs/vatz-plugin-eth2.0/policy"
...
func (s *pluginServer) Execute(ctx context.Context, req *pb.ExecuteRequest) (*pb.ExecuteResponse, error) {
	...
	case "IsValidatorUp":
		isUp, err := executor.IsValidatorUp()
		if err != nil {
			return nil, err
		}
...
func startServer(ch <-chan os.Signal) {
	log.Println("Start vatz-plugin-eth2.0")
